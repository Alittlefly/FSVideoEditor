...
- (void)createBaseUI {
    _logoImageView = [[UIImageView alloc] initWithFrame:CGRectMake(20, 12, 18, 18)];
    _logoImageView.image = [UIImage imageNamed:@"#"];
    [self addSubview:_logoImageView];
    
    _personNumLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.frame.size.width-20-100, 12, 100, 15)];
    ...
    _descripLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMinX(_titleLabel.frame), CGRectGetMaxY(_titleLabel.frame)+5, self.frame.size.width-40-12-5, 0)];
...
- (void)setChallengeModel:(FSChallengeModel *)challengeModel {
    if (challengeModel.challengeId == 0) {
        ...
        _titleLabel.frame = CGRectMake(CGRectGetMaxX(_logoImageView.frame)+5, 10, self.frame.size.width-40-CGRectGetWidth(_logoImageView.frame)-CGRectGetWidth(_addChallengeButton.frame)-10, 24);
        ...
        _titleLabel.frame = CGRectMake(CGRectGetMaxX(_logoImageView.frame)+5, 10, self.frame.size.width-40-CGRectGetWidth(_logoImageView.frame)-CGRectGetWidth(_personNumLabel.frame)-10, 24);
        _titleLabel.text = challengeModel.name;
        
        _descripLabel.text = challengeModel.content;
        [_descripLabel sizeToFit];
        _descripLabel.frame = CGRectMake(CGRectGetMinX(_titleLabel.frame), CGRectGetMaxY(_titleLabel.frame)+5, _descripLabel.frame.size.width, _descripLabel.frame.size.height);
    }
